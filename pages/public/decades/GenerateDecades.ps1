...
try {
    ...
    foreach ($decade in $decades) {
        ...
        $jsonFilePath = Join-Path $jsonBasePath $jsonFileName
        ...
        if (Test-Path $jsonFilePath) {
            ...
            $rankings = Get-Content -Path $jsonFilePath -Raw | ConvertFrom-Json

            # Check if rankings are not null and is an array
            if ($null -ne $rankings -and $rankings -is [System.Array]) {
                # Output the rankings for debugging
                Write-Host "Rankings for $($decade.DisplayName):" -ForegroundColor Yellow
                $rankings | Format-Table

                # Create table rows HTML
                $tableRows = ""
                foreach ($rank in $rankings | Select-Object -First 20) {
                    $tableRows += @"
                <tr>
                    <td>$($rank.rank)</td>
                    <td>$($rank.school)</td>
