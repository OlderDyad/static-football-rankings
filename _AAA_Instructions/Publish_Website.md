...
Write-Host $separator -ForegroundColor Cyan
Option 2: The Manual Step-by-Step Workflow
If you prefer to run each part manually for documentation, here are the exact steps.

Step 1: Generate All JSON Data
Navigate to your JSON scripts directory and run each one.
...
Navigate back to the main scripts folder and run the statistics generator.
...
From the same scripts folder, run the master page builder.
...
Navigate to the root of your repository and run the Git commands.

PowerShell

cd C:\Users\demck\OneDrive\Football_2024\static-football-rankings
git add .
git commit -m "Regenerate all data and HTML pages [Your Date/Message]"
git push origin main