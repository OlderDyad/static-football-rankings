...
$expectedFiles = @(
    (Join-Path $dataOutputDir "cumulative_games.html"),
    (Join-Path $dataOutputDir "annual_additions.html"),
    (Join-Path $dataOutputDir "statistics_summary.json")
)
...
if ($allFilesExist) {
    ...
    Write-Host "  2. Copy database_statistics.html to: $pageOutputDir" -ForegroundColor Gray
