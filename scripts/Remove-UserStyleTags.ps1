...
Write-Host "Finding files with  tags..." -ForegroundColor Yellow
...
if ($files.Count -eq 0) {
    Write-Host "No files with  tags found." -ForegroundColor Green
} else {
    Write-Host "Processed $($files.Count) files and removed  tags." -ForegroundColor Green
}
