...
const TableControls = {
   ...
   setupEventListeners() {
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('keyup', (e) => {  // Changed from 'input' to 'keyup'
...
// Update the filterTable function to be more robust:
filterTable(searchTerm) {
    ...
    if (!searchTerm || searchTerm.length === 0) {
        ...
        this.filteredRows = rows.filter(row => {
            const cellsText = Array.from(row.getElementsByTagName('td'))
                ...
                .join(' ')
                .toLowerCase();
            return cellsText.includes(searchTerm.toLowerCase());
        });
    }
    ...
    if (this.filteredRows.length === 0) {
        ...
        // Restore original rows before showing filtered ones
        tableBody.innerHTML = '';
        rows.forEach(row => tableBody.appendChild(row.cloneNode(true)));
        
        // Hide non-matching rows
        rows.forEach((row, index) => {
            row.style.display = this.filteredRows.includes(row) ? '' : 'none';
        });
