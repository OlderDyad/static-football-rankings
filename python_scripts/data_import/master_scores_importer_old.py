# master_scores_importer.py (v4 - with Smarter Score Sanitization)
...
def sanitize_score(score_text):
    """
    ** UPDATED FUNCTION **
    Cleans the raw score string. First, it replaces common character substitutions
    (like 'O' for '0'), then it removes any remaining non-digit characters.
    """
    if not isinstance(score_text, str):
        return ""
    
    # Step 1: Replace common substitutions like 'O' or 'o' with '0'.
    text = score_text.replace('O', '0').replace('o', '0')
    
    # Step 2: Now, remove any remaining characters that are not digits.
...
def main():
    logger.info("--- Starting Data Ingestion with Alias Verification (v4) ---")
