...
def generate_latest_season():
    ...
    # 1. Call Stored Procedure GetLatestSeasonTeams
    print("Calling GetLatestSeasonTeams procedure...", end=" ", flush=True)
    
    try:
        cursor.execute("EXEC GetLatestSeasonTeams @PageNumber=1, @PageSize=5000, @SearchTerm=NULL")
        ...
        latest_season = rankings[0].get('season', 2025) if rankings else 2025
        
        print(f"Done ({len(rankings)} teams for {latest_season}).")
    ...
    # 3. Merge Data
    final_items = []
    id_col = 'teamid' if 'teamid' in columns else ('id' if 'id' in columns else None)
    ...
    for rank_row in rankings:
        ...
        # State Logic
        state_val = get_string_value(rank_row, ['state'], '')
        if not state_val:
            state_val = meta['state'] if meta else ""
        if not state_val:
            state_val = extract_state_from_name(team_name)

        item = {
            "rank": get_int_value(rank_row, ['rank'], 0),
            "team": team_name,
            ...
            "hasTeamPage": get_int_value(rank_row, ['hasteampage'], 0) == 1,
            "teamPageUrl": get_string_value(rank_row, ['teampageurl'], '')
        }
    ...
    json_data = {
        "metadata": {
            ...
            "description": f"Rankings for {latest_season} Season"
        },
