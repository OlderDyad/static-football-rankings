...
def get_canadian_teams_from_db():
    ...
    try:
        ...
        # Get Canadian teams (ON, QC, NS, etc.)
        query = """
        SELECT DISTINCT 
            TeamName,
            State,
            COUNT(*) as GameCount
        FROM HS_Team_Names
        WHERE State IN ('ON', 'QC', 'NS', 'NB', 'AB', 'BC')
        GROUP BY TeamName, State
        ORDER BY State, TeamName
        """
        ...
        return [(row.TeamName, row.State, row.GameCount) for row in teams]
